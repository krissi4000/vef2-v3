...
//   authorId  Int
// }
app.put('/:id', zValidator('json', newsSchema, (result, c) => {
    if (!result.success) {
    ...
    const newsId = await prisma.news.findUnique({ where: { id: Number(id) }, });
    if (!newsId) {
        return c.json({ error: 'news not found' }, 404);
    }
...
//
//
app.post('/', zValidator('json', newsSchema, (result, c) => {
    if (!result.success) {
        return c.json({ error: 'bad request (zod error)' }, 400);
    }
}), async (c) => {
