...
app.put('/:id', zValidator('json', authorSchema, (result, c) => {
    if (!result.success) {
    ...
    const authorId = await prisma.author.findUnique({ where: { id: Number(id) }, });
    if (!authorId) {
        return c.json({ error: 'author not found' }, 404);
    }
...
app.post('/', zValidator('json', authorSchema, (result, c) => {
    if (!result.success) {
        return c.json({ error: 'bad request (zod error)' }, 400);
    }
}), async (c) => {
